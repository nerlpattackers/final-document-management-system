...
@section('content')
            ...
            <div class="col-md-6 d-flex justify-content-end">
                <nav aria-label="breadcrumb" class="">
                    <ol class="breadcrumb mb-0">
                      <li class="breadcrumb-item"><a href="{{ url('/incoming_documents') }}">Incoming Documents</a></li>
    ...
    <div class="container-fluid mb-3">
        <div class="row">
            <div class="col-6 d-flex align-items-center">
                <a href="{{ url('/incoming_documents') }}" class="text-dark back-btn"><i class="lni lni-chevron-left me-2"></i>Back</a>
            </div>
    ...
    @include('incoming.documents.addDocumentModal')
                    ...
                    <tbody>
                       ...
                       {{-- @if ($documents->user_id == auth()->user()->id) --}}
                            <tr>
                                ...
                                <td class="p-2">
                                    @if ($documents->to)
                                        @foreach(explode(',', $documents->to) as $to)
                                            <p class="text-reset">{{ trim($to) }}</p>
                                        @endforeach
                                    @else
                                        <p class="fst-italic">none</p>
                                    @endif
                                ...
                                <td class="p-2">
                                    @if ($documents->thru)
                                        @foreach(explode(',', $documents->thru) as $thru)
                                            <p class="text-reset">{{ trim($thru) }}</p>
                                        @endforeach
                                    @else
                                        <p class="fst-italic">none</p>
                                    @endif
                                ...
                                <td class="p-2">
                                    @if ($documents->from)
                                        @foreach(explode(',', $documents->from) as $from)
                                            <p class="text-reset">{{ trim($from) }}</p>
                                        @endforeach
                                    @else
                                        <p class="fst-italic">none</p>
                                    @endif
                                ...
                                <td class="p-2 text-center">
                                    <ul class="btn-group">
                                        <li>
                                            {{-- VIEW BUTTON --}}
                                            <a href="{{ url('/incoming_documents/' . $documents->id . '/view') }}" class="btn btn-primary me-2 action-btn d-flex justify-content-center align-items-center"><i class="lni lni-eye"></i></a>
                                        </li>
                                        <li>
                                            {{-- UPDATE BUTTON --}}
                                        ...
                                        @include('incoming.documents.updateDocumentModal')
                                        <li>
                                            {{-- DELETE BTN MODAL --}}
                                        ...
                                        @include('incoming.documents.deleteDocumentModal')
                                    </ul>
                                </td>
                            </tr>
                            {{-- @endif --}}
